...
from pytorch_pretrained_bert.optimization import WarmupCosineWithHardRestartsSchedule
...
PRETRAINED_DIR = os.path.join(SAVE_PATH, 'unet_efficientnet-b3_light_cls_seg_last.pt')
...
def main():
    ...
    scheduler = WarmupCosineWithHardRestartsSchedule(warmup=warmup_proportion,
...
#                                     verbosity=0)
    ...
    global_step = 0
    del checkpoint
    for epoch in tqdm(range(current_epoch, current_epoch + STAGE_EPOCHS)):
