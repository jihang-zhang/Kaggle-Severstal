...
def get_mean_and_std():
  if PRETRAINED_SET == 'imagenet':
    means = [0.485, 0.456, 0.406]
    stds = [0.229, 0.224, 0.225]
  elif PRETRAINED_SET == "cifar10":
    means = [0.49139968, 0.48215841, 0.44653091]
    stds = [0.24703223, 0.24348513, 0.26158784]
  elif PRETRAINED_SET == "svhn":
...
def create_cutout_mask(img_height, img_width, num_channels, size):
  ...
  mask = np.ones((img_height, img_width, num_channels), dtype=np.uint8)
  zeros = np.zeros((mask_height, mask_width, num_channels), dtype=np.uint8)
...
def apply_policy(policy, pil_img):
...
def _rotate_impl(pil_img, level, _):
  """Rotates `pil_img` from -15 to 15 degrees depending on `level`."""
  degrees = int_parameter(level, 15)
...
def _shear_x_impl(pil_img, level, img_shape):
  ...
  """
  level = float_parameter(level, 0.15)
  if random.random() > 0.5:
    level = -level
